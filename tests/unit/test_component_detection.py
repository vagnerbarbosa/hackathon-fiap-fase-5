...
class TestComponentDetectionService:
    ...
    async def test_detect_low_confidence(self, tmp_path):
        ...
        with patch("onnxruntime.InferenceSession", return_value=mock_session):
            ...
            assert (
                "baixa confiança" in str(exc_info.value).lower()
                or "não foi possível detectar" in str(exc_info.value).lower()
            )

    def test_default_confidence_threshold(self):
        """Deve ter threshold padrão de 0.15."""
        assert ComponentDetectionService.DEFAULT_CONFIDENCE_THRESHOLD == 0.15
    ...
    def test_service_initializes_with_custom_threshold(self, tmp_path):
        ...
        with patch("onnxruntime.InferenceSession", return_value=mock_session):
            service = ComponentDetectionService(
                model_path=str(model_path), confidence_threshold=0.5
            )
